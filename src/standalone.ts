...
function convertMarkdownToConfluenceStorage(markdown: string): string {
  ...
  html = html.replace(/<pre><code class="language-(.*?)">(.*?)<\/code><\/pre>/gs, (_match: any, lang: string, code: string) => {
  ...
  html = html.replace(/<pre><code>(.*?)<\/code><\/pre>/gs, (_match: any, code: string) => {
  ...
  html = html.replace(/<blockquote>([\s\S]*?)<\/blockquote>/g, '<ac:structured-macro ac:name="info"><ac:rich-text-body>$1</ac:rich-text-body></ac:structured-macro>');
...
function findConfluenceUrlInContent(content: string): string | null {
  const pattern = /^(?:\*\*|\*)?Confluence\s+reference(?:\*\*|\*)?\s*:\s*(https?:\/\/[^\s]+)/im;
...
function parseFrontmatter(content: string) {
  ...
  for (const line of lines) {
    ...
    const confluenceMatch = line.match(/^confluence:\s*(.+)$/);
    if (confluenceMatch) {
    ...
    if (currentKey === 'confluence' && line.match(/^\s+\w+:/)) {
      ...
      else if (key === 'spaceKey') frontmatter.spaceKey = value.replace(/['"]/g, '');
      else if (key === 'parentPageId') frontmatter.parentPageId = value.replace(/['"]/g, '');
      else if (key === 'title') frontmatter.title = value.replace(/['"]/g, '');
      else if (key === 'updateIfExists') frontmatter.updateIfExists = value.toLowerCase() === 'true';
    }
  }
...
class ConfluenceClient {
  ...
  constructor(config: { baseUrl: string; apiToken: string; email: string; spaceKey: string }) {
  ...
  async request(endpoint: string, method: string, body?: any): Promise<any> {
    ...
    if (!response.ok) {
      ...
      if (errorData && Object.keys(errorData).length > 0) {
        console.error(chalk.red('Confluence API error:'), JSON.stringify(errorData, null, 2));
      }
  ...
  async findPageByTitle(title: string, parentPageId?: string): Promise<any> {
    try {
      ...
      if (parentPageId) {
        const matchingPage = pages.find((page: any) => {
          ...
          return ancestors.some((ancestor: any) => ancestor.id === parentPageId);
        });
  ...
  async createPage(title: string, body: string, parentPageId?: string): Promise<any> {
    try {
  ...
  async updatePage(pageId: string, title: string, body: string, version: number): Promise<any> {
    try {
...
program
  ...
  .option('-s, --service <url>', 'Confluence base URL (overrides frontmatter and content)')
  .option('-k, --space <key>', 'Confluence space key (overrides frontmatter and content)')
  .option('-p, --parent <id>', 'Parent page ID (overrides frontmatter)')
  .option('-i, --page-id <id>', 'Page ID to update directly (bypasses title search)')
  .option('-t, --title <title>', 'Page title (overrides frontmatter, defaults to filename)')
  .option('-u, --update', 'Update page if exists (overrides frontmatter)', false)
  .action(async (markdownFile: string, options: any) => {
    try {
      ...
      if (!email || !apiToken) {
        console.error(chalk.red('❌ Missing required Confluence credentials in environment:'));
      ...
      if (urlToParse) {
        ...
        if (!spaceKey) {
          ...
          if (extractedSpace) {
            ...
            console.log(chalk.yellow(`ℹ️  Auto-detected space key "${spaceKey}" from URL`));
          }
        }
        ...
        if (!directPageId && !parentPageId) {
          ...
          if (extractedParentId) {
            ...
            console.log(chalk.yellow(`ℹ️  Auto-detected parent page ID "${parentPageId}" from URL`));
          }
        }
        ...
        if (!directPageId) {
          ...
          if (extractedPageId) {
            ...
            console.log(chalk.yellow(`ℹ️  Auto-detected page ID "${directPageId}" from URL`));
          }
        }
        if (!pageTitle) {
          ...
          if (extractedTitle) {
            ...
            console.log(chalk.yellow(`ℹ️  Auto-detected title "${pageTitle}" from URL`));
          }
        }
      }
      ...
      if (!baseUrl) {
        ...
        console.error(chalk.gray('   confluence: https://your-company.atlassian.net/wiki/spaces/SPACE/pages/123456789/Page+Title'));
        console.error(chalk.gray('   ---'));
      ...
      if (!spaceKey) {
        ...
        console.error(chalk.gray('   ---'));
        console.error(chalk.gray('   confluence: https://your-company.atlassian.net/wiki/spaces/SPACE/pages/123/Page+Title'));
        console.error(chalk.gray('   ---\n'));
        process.exit(1);
      }
      ...
      const spinner = ora('Converting Markdown to Confluence format...').start();
      ...
      if (updateIfExists) {
        ...
        if (existingPage) {
          ...
          const version = pageDetails.version.number;
          
          page = await client.updatePage(existingPage.id, pageTitle, htmlContent, version);
      ...
      const pageUrl = client.getPageUrl(page);
      console.log(chalk.blue(`\n🔗 Page URL: ${pageUrl}`));
